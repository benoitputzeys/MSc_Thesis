...
df_label = np.array(df_label).reshape(-1,1)
fig0, axs0=plt.subplots(1,1,figsize=(12,6))
axs0.plot(df_label)
fig0.show()

for i in range(1,len(df_label)-1):
    if (df_label[i, 0] > df_label[i - 1, 0] * 1.5) & (df_label[i, 0] > df_label[i + 1, 0] * 1.5):
        df_label[i, 0] = df_label[i - 1, 0]
    if (df_label[i, 0] < df_label[i - 1, 0] / 1.2) & (df_label[i, 0] < df_label[i + 1, 0] / 1.2):
            df_label[i, 0] = df_label[i - 1, 0]
fig2, axs2=plt.subplots(1,1,figsize=(12,6))
axs2.plot(df_label)
fig2.show()

x_axis = np.linspace(9400,9500, 100)
fig3, axs3=plt.subplots(1,1,figsize=(12,6))
axs3.plot(x_axis,df_label[9400:9500])
fig3.show()

# Determine the Features.
df_features = pd.DataFrame()
df_features["Total_Load_Past"] = df["Actual Total Load [MW] - United Kingdom (UK)"].shift(+48*7)
...
np.savetxt("Data_Entsoe/Data_Preprocessing/For_336_SP_Step_Prediction/X.csv", X, delimiter=",")
np.savetxt("Data_Entsoe/Data_Preprocessing/For_336_SP_Step_Prediction/y.csv", y, delimiter=",")
#
