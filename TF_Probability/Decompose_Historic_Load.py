...
for i in range(1,round(len(series)/336),1):
    mean_each_week[144+336*i-336:144+336*i] = np.mean(series[144+336*i-336:144+336*i])
mean_each_week[-278:] = np.mean(series[-278:])
mean_each_week[:144] = np.mean(series[:144])
...
axs[0].plot(dates[-48*7:],daily_seasonality[-48*7:], color = "blue", linewidth = 0.5)
axs[0].set_ylabel("Daily [GW]", size = 12)
axs[1].plot(dates[-48*7:],weekly_seasonality[-48*7:], color = "blue", linewidth = 0.5)
axs[1].set_ylabel("Weekly [GW]", size = 12)
axs[2].plot(dates[-48*7:],mean_each_week[-48*7:], color = "blue")
axs[2].set_ylabel("Trend [GW]", size = 12)
axs[3].plot(dates[-48*7:], residual[-48*7:] , color = "blue", linewidth = 0.5)
axs[3].set_ylabel("Residual [GW]", size = 12)
axs[3].set_xlabel("Date", size = 14)
...
axs2[0].plot(dates[-48*7:], modified_recons[-48*7:], color = "blue", linewidth = 0.5)
axs2[0].set_ylabel("Modified Reconstruction [GW]", size = 12)
axs2[1].plot(dates[-48*7:], series[-48*7:], color = "blue", linewidth = 0.5)
axs2[1].set_ylabel("Actual [GW]", size = 14)
axs2[2].plot(dates[-48*7:],(modified_recons-series)[-48*7:], color = "blue", linewidth = 0.5)
...
projected_load = modified_recons.iloc[:, 1]
...
axs3.scatter(settlement_period, projected_load, alpha=0.05, label = "Projected Loads", color = "blue")
...
axs5.plot(settlement_period[-336:], (df_stats.iloc[:,1]+mean), color = "blue", label = "Mean of all projected loads")
axs5.plot(settlement_period[-336:], series[-48*7:], color = "red", label = "Actual Load of most recent week")
axs5.fill_between(df_stats.iloc[:,0], ((df_stats.iloc[:,1]-df_stats.iloc[:,2])+mean), ((df_stats.iloc[:,1]+df_stats.iloc[:,2])+mean),alpha=0.2, color = "blue", label = "Stddev")
