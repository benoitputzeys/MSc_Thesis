...
fig2.suptitle('Random Forest: Training Set', fontsize=16)
ax2[0].plot(X_train_unscaled[:,0],linewidth=0.5)
ax2[0].set_xlabel('Settlement Period')
ax2[0].set_ylabel('Actual Value: Training Set')

ax2[1].plot( result_train, linewidth=0.5)
ax2[1].set_xlabel('Settlement Period')
ax2[1].set_ylabel('Prediction on training set')

ax2[2].plot(abs(error_train), linewidth=0.5)
...
import csv
with open('Compare_Models/Single_Step_Results/Random_Forest_result.csv', 'w', newline='', ) as file:
