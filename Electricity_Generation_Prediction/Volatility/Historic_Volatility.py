...
axes1[0].plot(create_dates(X_train,y_train),label = "Actual historic values", linewidth=0.5)
axes1[0].plot(create_dates(X_train,mean), label = "Mean of historic values",color = 'orange', linewidth=0.5)
axes1[0].set_xlabel('Settlement Period Training Set')
...
axes1[1].set_xlabel('Settlement Period Training Set')
axes1[1].set_ylabel('Difference (Mean-Actual) [MW]')

axes1[2].plot(create_dates(X_train,y_train-mean)**2,label = "Squarred Difference",color = 'red', linewidth=0.5)
axes1[2].set_xlabel('Settlement Period Training Set')
...
fig2.subtitle = ("Actual Values and Volatility")
axes2[0].plot(create_dates(X_train[-48*3:],y_train[-48*3:]), label = "Actual historic values", linewidth=0.5)
axes2[0].fill_between(dates,  y_train[-48*3:][:,0] - np.std(y_train), y_train[-48*3:][:,0] + np.std(y_train), alpha=0.1)
axes2[0].plot(create_dates(X_train[-48*3:],mean_zoom), label = "Mean train set", linewidth=0.5)
axes2[0].set_xlabel('Settlement Period')
axes2[0].set_ylabel('Electricity Load [MW]')

axes2[1].plot(create_dates(X_train[-48*3:],y_train[-48*3:] - mean), label = "Difference between mean and actual load",color = 'black', linewidth=0.5)
axes2[1].set_xlabel('Settlement Period')
axes2[1].set_ylabel('Difference (Mean-Actual) [MW]')

axes2[2].plot(create_dates(X_train[-48*3:],y_train[-48*3:]-mean)**2,label = "Squared Difference",color = 'red', linewidth=0.5)
axes2[2].set_xlabel('Settlement Period')
axes2[2].set_ylabel('Squared Difference')
fig2.legend()
fig2.show()
...
fig3, axes3 = plt.subplots(3)
fig3.subtitle = ("Actual Values and Volatility")
y_values = create_dates(X_train,y_train)
axes3[0].plot(y_values,label = "Actual historic values", linewidth=0.5)
axes3[0].fill_between(dates,  y_train[:,0]-np.std(y_train),  y_train[:,0]+np.std(y_train),alpha=0.1)
y_values = create_dates(X_train,mean_zoom)
axes3[0].set_xlabel('Settlement Period')
axes3[0].set_ylabel('Electricity Load [MW]')

axes3[1].plot(create_dates(X_train,y_train-mean),label = "Difference between mean and actual load",color = 'black', linewidth=0.5)
axes3[1].set_xlabel('Settlement Period')
axes3[1].set_ylabel('Difference')

axes3[2].plot(create_dates(X_train,y_train-mean)**2,label = "Squared Difference",color = 'red', linewidth=0.5)
axes3[2].set_xlabel('Settlement Period')
axes3[2].set_ylabel('Squared Difference')
fig3.legend()
fig3.show()
