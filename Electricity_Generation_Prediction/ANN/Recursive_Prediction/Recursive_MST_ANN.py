...
for i in range(0,48*7):
    ...
    result_future = np.append(result_future, y_scaler.inverse_transform(my_model.predict(x_scaler.transform(update_row).reshape(1,7))))
...
axs2[0].plot(dates.iloc[-len(X_test):-len(X_test)+48*7],
             ...
             label = "ANN Recursive Prediction", color = "orange")
axs2[0].plot(dates.iloc[-len(X_test):-len(X_test)+48*7],
             y_test[:48*7],
