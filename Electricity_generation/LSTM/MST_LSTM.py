...
for i in range(0,48*7):
    ...
    result_future = np.append(result_future, y_scaler.inverse_transform(my_model.predict(x_scaler.transform(update_row).reshape(1,10,1))))
...
plt.title('Prediction 7 days in the future with LSTM')
...
y_values_dates = create_dates(X_future_features[-48*7:].to_numpy(),result_future)
axes2[0].plot(y_values_dates, linewidth=0.5, label ="Prediction 7 days in the future with LSTM")
...
pd.DataFrame(result_future).to_csv("/Users/benoitputzeys/PycharmProjects/MSc_Thesis/Electricity_generation/Hybrid_Model/LSTM_prediction.csv")
