...
# Decompose the data.
ts_decompose = sm.tsa.seasonal_decompose(y_train_1, model='additive', period = 48)
ts_decompose.plot()
plt.show
...
predictions_train_1 = model_fit.predict(start = 1, end = len(X_train_1))
...
print("The mean absolute percent error of the test set is %0.2f" % np.mean(abs((y_train_1[-48*7:]-predictions_train_1[-48*7:])/y_train_1[-48*7:])))
print("-"*200)
...
one = np.array(predictions_train_1[:48*14].reset_index())
two = np.array(predictions_train_1[48:48*15].reset_index())
