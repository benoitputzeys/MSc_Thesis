...
predictions_train = model_fit.predict(start = 1, end = len(X_train))
...
print("The mean absolute percent error of the test set is %0.2f" % np.mean(abs((y_train[-48*7:]-predictions_train[-48*7:])/y_train[-48*7:])))
print("-"*200)
...
#y_values_dates = create_dates(X_train_2[-48*7:], y_train_2[-48*7:])
ax[0].plot(y_train, label='Actual Values')
#y_values_dates = create_dates(X_train_2[-48*7:], predictions_train_2[-48*7:])
ax[0].plot(predictions_train, label='Predictions')
...
predictions_train = model_fit.forecast(steps = len(X_train) )
predictions_train = np.reshape(predictions_train,(-1,1))
residuals = y_train[:len(X_train)] - predictions_train
...
print("The mean absolute percent error of the train set 2 is %0.2f" % np.mean(abs((y_train[-48*7:]-predictions_train[-48*7:])/y_train[-48*7:])))
print("-"*200)
...
pd.DataFrame(predictions_train).to_csv(
    "/Users/benoitputzeys/PycharmProjects/MSc_Thesis/Electricity_generation/Hybrid_Model/Pred_train2_other_metrics/SARIMA_prediction.csv")
