...
# Only include half the length of the training and the test set.
...
my_model.save("Load_Prediction/ANN/Direct_Multi_Step_Prediction/Model_1.h5")
#my_model = keras.models.load_model("Load_Prediction/ANN/Direct_Multi_Step_Prediction/Model_1.h5")
...
# Include a column vector that contains the errors between the prediction and the test set.
error_test_plot = np.zeros((48*3+48*7,1))
error_test_plot[-336:] = error_test[:48*7].reshape(-1,1)

# Plot the result with the values of the training set in blue and the values in the test ste in black
...
axs2[0].plor(30,30, label = "Error",color = "red")

axs2[1].plot(dates.iloc[-len(X_test)-48*3:-len(X_test)+48*7],
             error_test_plot, color = "red")
...
df_errors.to_csv("Load_Prediction/ANN/Impact_of_Random_Initialization/Results/NN_error_Model_1.csv")
