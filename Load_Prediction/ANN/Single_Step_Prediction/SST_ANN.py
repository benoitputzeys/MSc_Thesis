...
my_model.save("Load_Prediction/ANN/Single_Step_Prediction/SST_ANN_model.h5")
#my_model = keras.models.load_model("Load_Prediction/ANN/Single_Step_Prediction/SST_ANN_model.h5")
...
# Compute the error between the load and the prediction from the ANN
print("-"*200)
...
fig2.suptitle('Test Set', fontsize=16)
# Plot the actual generation in a new subplot of 3x1.
plot_actual_generation(axes1, X[len(X)-len(y_test):,:], y[-len(y_test):], "Test set")
# Plot the the predicted (NN) generation.
plot_predicted_generation(axes1, X[len(X)-len(result_test):,:], result_test, "NN prediction")
# Plot the error between the predicted and the actual temperature.
plot_error(axes1, X[len(X)-len(error_test):,:], error_test, "Error")
fig2.show()
