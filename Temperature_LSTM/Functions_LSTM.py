...
def create_model(input_variable, learning_rate):
    ...
    my_model.add(LSTM(units=20, return_sequences=True, input_shape=(input_variable.shape[1], input_variable.shape[2]),
    ...
    # previous layer.
    my_model.add(LSTM(units=20, kernel_initializer = initializer))
    my_model.add(Dropout(0.2))
...
def plot_loss(epochs, difference, counter):
