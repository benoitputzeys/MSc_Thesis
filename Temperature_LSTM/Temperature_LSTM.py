...
regressor.add(LSTM(units = 50, return_sequences = True, input_shape = (X_train.shape[2], 4)))
...
for j in range(3):
    for i in range(30, len(inputs)):
        np.append(X_test, inputs[i-30:i, j], axis = j)
...
error = test_temperatures - predicted_temperature
print("The mean absolute error of the prediciton is ", np.sum(error)/len(error))
