...
dates = np.array([dt.datetime.strptime(df.iloc[i,0][0:19], '%Y-%m-%d %H:%M:%S') for i in range(len(df))])
df["Timestamp"] = dates
...
X.to_csv("Data_Entsoe/Data_Preprocessing/For_Multi_Step_Prediction/X_API.csv")
y.to_csv("Data_Entsoe/Data_Preprocessing/For_Multi_Step_Prediction/y_API.csv")
#
